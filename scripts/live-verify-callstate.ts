/**
 * scripts/live-verify-callstate.ts
 ...
 *
 * Run:  node dist-probe/scripts/live-verify-callstate.js
 */

import { writeFileSync } from "node:fs";
import { join } from "node:path";
import { runEscalationAgent } from "../packages/agent";
import type { AgentDependencies } from "../packages/agent/graph";
import type { EscalationContext, Responder } from "../packages/types";
...
async function instrumentSdk() {
  const { getCalle } = await import("../packages/calle/client");
...
async function main() {
  ...
  // ── Findings ───────────────────────────────────────────────────────────────
  const outPath = join(__dirname, "..", "..", "docs", "live-call-raw.json");
  writeFileSync(
    outPath,
