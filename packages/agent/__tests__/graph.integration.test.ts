...
describe("graph: live call streaming (FR-5.2)", () => {
  test("emits the call.state sequence a REAL call produces", async () => {
    const { deps, spy } = makeDeps("accept_after_pushback");
    await runEscalationAgent(CONTEXT, deps);

    // Verified against live calls (F-003, F-006): `connected` never fires, and
    // `in_conversation` is effectively skipped because transcript turns only
    // arrive at completion. The dashboard must not depend on either.
    expect(spy.callStates).toEqual(["queued", "dialling", "extracting", "completed"]);
    expect(spy.callStates).not.toContain("connected");
  }, 30_000);

  test("the optimistic sequence is opt-in only", async () => {
    process.env.SENTINEL_MOCK_OPTIMISTIC = "true";
    try {
      const { deps, spy } = makeDeps("accept_after_pushback");
      await runEscalationAgent(CONTEXT, deps);
      expect(spy.callStates).toContain("connected");
      expect(spy.callStates).toContain("in_conversation");
    } finally {
